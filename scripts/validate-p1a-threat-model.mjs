...
export function validateData(model, evidence, manifest, markdown, opts = {}) {
  ...
  assert.deepEqual(summary,{actors:model.actors.length,actions:model.actions.length,authorityRules:model.actors.length*model.actions.length,assets:model.assets.length,trustBoundaries:model.boundaries.length,dataFlows:model.flows.length,threats:model.threats.length,controls:model.controls.length,tenantOperations:model.tenantOperations.length,credentialClasses:model.credentialClasses.length,founderDecisions:model.founderDecisions.length,sourceEvidenceReferences:evidence.references.length},"Markdown/JSON count mismatch");
...
function main(){
  ...
  try {
    const out=runPackage({candidateSha:candidate,runGit:true});
    for(const name of names){totals.executed++;totals.passed++;console.log(`PASS ${name}`);}
    console.log(JSON.stringify({suite:"p1-a-threat-model",candidateSha:out.head,...totals,authorityRules:out.authorityRules,threats:out.model.threats.length,controls:out.model.controls.length}));
  } catch(error) {
    totals.executed++;totals.failed++;
    console.error(`FAIL ${names[0]}: ${error.message}`);
    console.error(JSON.stringify({suite:"p1-a-threat-model",candidateSha:candidate??null,...totals}));
    process.exitCode=1;
  }
}
