...
export function verifyEventBoundAmendmentTopology({
  ...
  assert.ok(["pull_request", "push"].includes(eventName),
  ...
  const headParents = gitAt(resolved, "show", "-s", "--format=%P", eventHeadSha)
    .split(" ").filter(Boolean);
  let secondParentSha;
  let subjectClass;
  if (eventName === "pull_request") {
    assert.deepEqual(headParents, [eventBaseSha],
      "event authority: PR amendment must have exactly the event base as parent");
    secondParentSha = eventHeadSha;
    subjectClass = "EVENT_BOUND_PR_AMENDMENT";
  } else {
    ...
    assert.deepEqual(
      gitAt(resolved, "show", "-s", "--format=%P", secondParentSha).split(" ").filter(Boolean),
      [eventBaseSha],
...
const TRUSTED_CURRENT_CONTRACT_ADDITION = [
  ...
  "          if test \"$P1A_EVENT_NAME\" = pull_request || test \"$P1A_EVENT_BASE_SHA\" != \"$zero_sha\"; then",
  ...
  "            git -C \"$authority\" -c protocol.version=2 fetch --no-tags --no-write-fetch-head \\",
  "              --depth=2 origin \"$P1A_EVENT_BASE_SHA\" \"$P1A_EVENT_HEAD_SHA\"",
  ...
  "          else",
  "            unset P1A_EVENT_REPOSITORY P1A_EVENT_NAME P1A_EVENT_BASE_REPOSITORY",
  "            unset P1A_EVENT_HEAD_REPOSITORY P1A_EVENT_BASE_REF P1A_EVENT_BASE_SHA P1A_EVENT_HEAD_SHA",
  "          fi",
  "          node scripts/test-p1a-dual-base-verifier.mjs",
  "          if test -n \"${P1A_EVENT_AUTHORITY_ROOT:-}\"; then",
  "            rm -rf -- \"$authority\"",
  "            test ! -e \"$authority\"",
  "            trap - EXIT",
  "          fi",
