...
function parseArgs(argv: string[]): { approve: boolean; reason?: string; controlsPath: string; off: boolean } {
...
function main(): void {
  ...
  const active = !cli.off;
  const next = {
    ...readControls(cli.controlsPath),
    runtime_kill_switch: active,
  ...
  console.log(JSON.stringify({ ok: true, controls: cli.controlsPath, runtime_kill_switch: active }, null, 2));
}
