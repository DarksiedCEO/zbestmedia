...
export async function buildServer(envInput?: AppEnv): Promise<FastifyInstance> {
  ...
  const aaliyahSignalCoalescingService = new AaliyahSignalCoalescingService(
    agentRepository,
    aaliyahDiagnosticsService
  );
  const aaliyahEscalationEngineService = new AaliyahEscalationEngineService(
    agentRepository,
    aaliyahDiagnosticsService,
    aaliyahFounderPreferencesService.resolver
  );
  const aaliyahOperatorQueueService = new AaliyahOperatorQueueService(
    agentRepository,
    aaliyahDiagnosticsService
  );
  ...
  const aaliyahOutcomeFeedbackService = new AaliyahOutcomeFeedbackService(
    agentRepository,
    aaliyahDiagnosticsService
  );
  const aaliyahFounderBriefService = new AaliyahFounderBriefService(
    agentRepository,
    aaliyahDiagnosticsService,
    aaliyahFounderPreferencesService.resolver
  );
  const aaliyahTimelineService = new AaliyahTimelineService(
    agentRepository,
    aaliyahDiagnosticsService
  );
