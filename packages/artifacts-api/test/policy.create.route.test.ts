...
describe("policy firewall on create route", () => {
  ...
  it("returns 400 policy_denied for forbidden artifact type", async () => {
  ...
  it("returns 400 policy_denied for forbidden phrase in payload", async () => {
    const token = await signToken("11111111-1111-4111-8111-111111111111");
    const res = await app.inject({
      method: "POST",
      url: "/v1/artifacts",
      ...
      payload: {
        ...
        sourceArtifactIds: [],
        evalReport: { gates: [{ gateId: "G1", passed: true }], summary: "ok" },
        payload: { claim: "These are GUARANTEED RESULTS for every client." }
      }
    });
