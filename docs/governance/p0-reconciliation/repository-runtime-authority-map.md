# Repository and Runtime Authority Map

## Provisional authority

| Domain | Provisional authority | Allowed claim | Forbidden claim |
|---|---|---|---|
...
| Search Intelligence | Separate repository, path/remote not authorized | Architectural reservation only | Implementation in either existing repository |
| Master Sentinel | Separate repository, path/remote not authorized | External trust-boundary reservation only | Local substitute or self-certification |

## Runtime boundary

`zbestmedia` defines an approved, versioned contract. `zbestmedia-ui` binds that exact contract to an executable implementation. Runtime evidence must refer to both immutable commits.
...
These conflicts are not resolved by this candidate. No clone or branch may be deleted, rebased, or relabeled from this document.
